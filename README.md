...
Seed gradient-based fallback icons for every record in `public.profiles` by uploading generated PNGs to Supabase Storage and writing the public URL into `icon_storage`:
...
The `public` schema is accessed securely through PostgreSQL RPC functions in the `public` schema:
...
These functions use `SECURITY DEFINER` to provide controlled access without exposing the schema directly. See `supabase/migrations/expose_public_via_rpc.sql` for the complete implementation.
