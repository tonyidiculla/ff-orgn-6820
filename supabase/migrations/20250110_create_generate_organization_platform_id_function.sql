...
CREATE OR REPLACE FUNCTION public.generate_organization_platform_id()
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    LOOP
        ...
        IF NOT EXISTS (SELECT 1 FROM public.organizations WHERE organization_platform_id = v_platform_id) THEN
...
GRANT EXECUTE ON FUNCTION public.generate_organization_platform_id() TO authenticated;

-- Add comment
COMMENT ON FUNCTION public.generate_organization_platform_id IS 
