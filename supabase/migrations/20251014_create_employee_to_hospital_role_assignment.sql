...
-- This table links employees (users) to specific hospital_master with their roles
-- Part of the complete authentication flow:
-- auth.users -> public.profiles -> public.user_to_role_assignment -> public.employee_to_hospital_role_assignment

-- Create the table in public schema
CREATE TABLE IF NOT EXISTS public.employee_to_hospital_role_assignment (
    ...
    -- Foreign key to hospital_master table
    CONSTRAINT fk_hospital_id 
        FOREIGN KEY (hospital_id) 
        REFERENCES public.hospital_master(entity_platform_id)
        ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_employee_hospital_employee 
    ON public.employee_to_hospital_role_assignment(employee_platform_id);

CREATE INDEX IF NOT EXISTS idx_employee_hospital_hospital 
    ON public.employee_to_hospital_role_assignment(hospital_id);

CREATE INDEX IF NOT EXISTS idx_employee_hospital_active 
    ON public.employee_to_hospital_role_assignment(is_active) 
    WHERE is_active = true;

-- Create updated_at trigger
CREATE OR REPLACE FUNCTION public.update_employee_hospital_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER trigger_update_employee_hospital_updated_at
    BEFORE UPDATE ON public.employee_to_hospital_role_assignment
    FOR EACH ROW
    EXECUTE FUNCTION public.update_employee_hospital_updated_at();

-- Enable Row Level Security
ALTER TABLE public.employee_to_hospital_role_assignment ENABLE ROW LEVEL SECURITY;

-- RLS Policy: Allow authenticated users to view their own assignments
CREATE POLICY "employee_hospital_select_own" 
    ON public.employee_to_hospital_role_assignment
    FOR SELECT
    TO authenticated
    USING (
...
CREATE POLICY "employee_hospital_select_hospital_admin" 
    ...
    USING (
        hospital_id IN (
            ...
            FROM public.employee_to_hospital_role_assignment ehra
            JOIN public.profiles p ON ehra.employee_platform_id = p.user_platform_id
            JOIN public.user_to_role_assignment ura ON p.user_platform_id = ura.user_platform_id
            JOIN public.platform_roles pr ON ura.platform_role_id = pr.id
            WHERE p.user_id = auth.uid()
...
-- Grant permissions
GRANT SELECT ON public.employee_to_hospital_role_assignment TO authenticated;
GRANT INSERT, UPDATE, DELETE ON public.employee_to_hospital_role_assignment TO authenticated;
GRANT SELECT ON public.employee_to_hospital_role_assignment TO anon;

-- Add helpful comment
COMMENT ON TABLE public.employee_to_hospital_role_assignment IS 
'Links employees to specific hospital_master with their roles and assignments. Part of the complete authentication flow.';
