...
CREATE OR REPLACE FUNCTION public.generate_entity_platform_id()
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    LOOP
        ...
        SELECT EXISTS(
            SELECT 1 FROM public.hospital_master WHERE entity_platform_id = v_platform_id
        ) INTO v_exists;
...
GRANT EXECUTE ON FUNCTION public.generate_entity_platform_id() TO authenticated;

-- Add comment
COMMENT ON FUNCTION public.generate_entity_platform_id IS 
'Generates a unique entity platform ID in format: E01 + 6 random alphanumeric characters
Example: E01x3Bm9k
Checks hospital_master table for uniqueness.';
