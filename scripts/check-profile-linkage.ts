...
async function checkProfileLinkage() {
    ...
    console.log(`   INSERT INTO public.user_to_role_assignment (user_platform_id, platform_role_id, is_active)`)
    console.log(`   VALUES ('${profile.user_platform_id}',`)
        console.log(`           (SELECT id FROM public.platform_roles WHERE role_name = 'platform_admin' LIMIT 1),`)
        console.log(`           true);`)
        return
    }
    ...
    console.log(`   → public.user_to_role_assignment.user_platform_id`)
