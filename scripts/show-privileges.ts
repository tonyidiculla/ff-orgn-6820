...
async function findMyPrivileges() {
    ...
    try {
        ...
        if (result.rows.length === 0) {
            console.log('⚠️  No active role assignments found in public.user_to_role_assignment')
            console.log('\n💡 To assign yourself a role, run SQL like:')
            console.log(`
INSERT INTO public.user_to_role_assignment (user_id, platform_role_id, is_active)
VALUES 
  ('YOUR_USER_ID', (SELECT id FROM public.platform_roles WHERE role_name = 'platform_admin'), true);
            `)
            return
        }
